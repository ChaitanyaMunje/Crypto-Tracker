...
public class MainActivity extends AppCompatActivity {
    ...
    //creating variable for recycler view, adapter, array list, progress bar
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getData();
        //on below line we are adding text watcher for our edit text to check the data entered in edittext.
    ...
    private void filter(String filter) {
        //on below line we are creating a new array list for storing our filtered data.
        ...
        for (CurrencyModal item : currencyModalArrayList) {
            //on below line we are getting the item which are filtered and adding it to filtered list.
            if (item.getName().toLowerCase().contains(filter.toLowerCase())) {
                filteredlist.add(item);
            }
        }
        //on below line we are checking weather the list is emoty or not.
    ...
    private void getData() {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET, url, null, new Response.Listener<JSONObject>() {
            ...
            public void onErrorResponse(VolleyError error) {
                //displaying error response when recieved any error.
                Toast.makeText(MainActivity.this, "Something went amiss. Please try again later", Toast.LENGTH_SHORT).show();
            }
        }) {
            @Override
            public Map<String, String> getHeaders() {
                ...
                headers.put("X-CMC_PRO_API_KEY", "Emter your API key");
                //at last returning headers.
                return headers;
            }
        };
        //calling a method to add our json object request to our queue.
